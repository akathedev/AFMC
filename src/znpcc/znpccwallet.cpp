...
CzAFMCWallet::CzAFMCWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        ...
        if (walletdb.ReadZAFMCSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAFMCSeed_deprecated()) {
...
bool CzAFMCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAFMCCount(nCountLastUsed);
    else if (!walletdb.ReadZAFMCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAFMCWallet::Lock()
{
    seedMaster = 0;
}

void CzAFMCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAFMCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAFMC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAFMCWallet::LoadMintPoolFromDB()
{
...
void CzAFMCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAFMCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAFMCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzAFMCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAFMC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzAFMCWallet::SeedToZAFMC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAFMCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAFMCWallet::GenerateDeterministicZAFMC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAFMCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAFMCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAFMCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
