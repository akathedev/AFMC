...
CzAFMCTracker::CzAFMCTracker(std::string strWalletFile)
{
...
CzAFMCTracker::~CzAFMCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAFMCTracker::Init()
{
...
bool CzAFMCTracker::Archive(CMintMeta& meta)
{
...
bool CzAFMCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAFMCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAFMCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAFMCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAFMCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAFMCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAFMCTracker::GetSerialHashes()
{
...
CAmount CzAFMCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAFMCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAFMCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAFMCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAFMCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAFMCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAFMCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAFMCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAFMCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAFMCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAFMCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAFMCWallet* zAFMCWallet)
{
    bool iszAFMCWalletInitialized = (NULL != zAFMCWallet);
    CMintMeta meta;
    ...
    if (! iszAFMCWalletInitialized)
        zAFMCWallet = new CzAFMCWallet(strWalletFile);
    meta.isSeedCorrect = zAFMCWallet->CheckSeed(dMint);
    if (! iszAFMCWalletInitialized)
        delete zAFMCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAFMCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAFMCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAFMCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAFMCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAFMCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAFMCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAFMCWallet* zAFMCWallet = new CzAFMCWallet(strWalletFile);
