...
#include "ui_zafmccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zafmcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zafmcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zAFMC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zAFMC seed used to mint this zAFMC is not the same as currently hold in the wallet";
            else
