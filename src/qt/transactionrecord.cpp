...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZAFMCModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zafmcTracker->HasMintTx(hash))) {
            //zAFMC stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZAFMC;
...
bool IsZAFMCType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZAFMC:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZAFMC || type == TransactionRecord::MNReward) {
