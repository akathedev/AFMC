...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 AFMC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAFMC send
enum ZerocoinSpendStatus {
    ZAFMC_SPEND_OKAY = 0,                            // No error
    ZAFMC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAFMC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAFMC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAFMC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAFMC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAFMC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAFMC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAFMC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAFMC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAFMC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAFMC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAFMC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAFMC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAFMC_SPENT_USED_ZAFMC = 14,                      // Coin has already been spend
    ZAFMC_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZAFMC_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZAFMCOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    /** zAFMC reset */
    boost::signals2::signal<void()> NotifyzAFMCReset;
