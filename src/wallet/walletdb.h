...
public:
    ...
    bool WriteZAFMCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZAFMCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZAFMCSeed_deprecated(uint256& seed);
    bool EraseZAFMCSeed();
    bool EraseZAFMCSeed_deprecated();

    bool WriteZAFMCCount(const uint32_t& nCount);
    bool ReadZAFMCCount(uint32_t& nCount);
